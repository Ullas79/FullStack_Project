...
const JWT_SECRET =
  ...
  "your-super-secret-key-that-should-be-in-a-env-file";

// --- REPLACE WITH YOUR CONNECTION STRING ---
const DB_URL =
  "mongodb+srv://ullasmogaveera2004_db_user:uowuhUZmRFsFLBXG@m0.jfaouna.mongodb.net/?retryWrites=true&w=majority&appName=M0";
...
app.put("/api/documents/:id/rename", auth, async (req, res) => {
  try {
    ...
    if (document.owner.toString() !== loggedInUserId) {
      return res
        .status(403)
        .json({ message: "Only the document owner can rename" });
    }
...
app.delete("/api/documents/:id", auth, async (req, res) => {
  try {
    ...
    if (document.owner.toString() !== loggedInUserId) {
      return res
        .status(403)
        .json({ message: "Only the document owner can delete" });
    }
...
app.post("/api/documents/:id/unshare", auth, async (req, res) => {
  try {
    ...
    if (document.owner.toString() !== loggedInUserId) {
      return res
        .status(403)
        .json({ message: "Only the document owner can manage sharing" });
    }
    ...
    const isCollaborator = document.collaborators.some((id) =>
      id.equals(userToRemoveId),
    );
    if (!isCollaborator) {
      return res
        .status(400)
        .json({ message: "User is not a collaborator on this document" });
    }
